...
const METHODS = ["Bank Transfer", "QRIS", "BCA Virtual Account", "BNI Virtual Account"];
...
const deposits: Deposit[] = Array.from({ length: 27 }, (_, i) => {
  const day = 28 - (i % 28);
  return {
    id: String(i + 1),
    datetime: `2026-08-${pad(day)} ${pad(8 + (i % 12))}:${pad((i * 17) % 60)}`,
    code: `DP-${String(100394 - i).padStart(6, "0")}`,
...
export function DepositHistory() {
  ...
  const rows = useMemo(() => {
    ...
    return deposits.filter((d) => {
      ...
      if (query && !`${d.code} ${d.method}`.toLowerCase().includes(query)) return false;
      return true;
    });
  ...
  return (
    <Card className="shadow-sm">
      <CardHeader>
        <CardTitle className="text-lg font-semibold text-primary">Deposit History</CardTitle>
      </CardHeader>
